...
master_node_password=os.environ["KUBERNETES_MASTER_PASSWORD"]
master_node_hostname=os.environ["KUBERNETES_MASTER_HOSTNAME"]
master_node_ip=os.environ["KUBERNETES_MASTER_IP"]
token_k8s=os.environ["KUBERNETES_MASTER_TOKEN"]

host="https://"+master_node_ip+":16443"
...
def add_node(node_info):
    ...
    if len(lines_check)==0:
        ...
        stdin_check, stdout_check, stderr_check =ssh.exec_command("echo '"+rmaster_node_password+"' | sudo -S -k cat hosts new_host >> hosts")
    ...
    if any("The node has joined the cluster" in word for word in lines_worker):
        ...
        stdin, stdout, stderr = ssh.exec_command("echo '"+master_node_password+"' | sudo -S -k microk8s.kubectl label nodes "+node_info["name"]+ " location="+node_info["location"])
...
def delete_service_function(service_function_name):
    ...
    for volume in volume_list.items:
        ...
        if name_v in volume.metadata.name:
            deleted_volume = v1.delete_namespaced_persistent_volume_claim(
...
def create_pv_dict(name, volumes):
    name_vol = name + str("-") + volumes["name"]
