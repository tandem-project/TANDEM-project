...
function gotopage_v1(name,c){
    ...
    let time = d.getTime();
    if (!_LEAVENOW)
    if (confirm("Your updates will be lost. Are you sure you want to leave this page?") === false) {
        return;
    }
    _LEAVENOW = true;
//if (_COMPANYNAME==""){
...
function _makeicons(jsicontxt){
        var jsicon = jsicontxt;
        jsicon["tdstyle"]="padding-bottom: 3px;padding-top: 2px;border: 1px solid black;border-radius: 4px;box-shadow: 2px 5px 5px #9e9e9e;border-style: outset;";
        var restxt = "<table>";
        for (var i= 0; i < jsicon.rows.length; i++){
            restxt += "<tr>";
            for (var j= 0; j < jsicon.rows[i].length; j++){
                restxt += "<td style='"+jsicon.tdstyle+"'><a href='#'>" +jsicon.rows[i][j];
                restxt += "</a></td>";
            }
            restxt += "</tr>";
        }
